...
const Cart = () => {
    ...
    try {
      ...
      if (res.status === 200 && res.data?.products) {
        const cartItems = res.data.products.map((item) => ({
          ...
          productId: {
            ...item.productId,
            productName: DOMPurify.sanitize(item.productId.productName || ""),
            productDescription: DOMPurify.sanitize(item.productId.productDescription || ""),
            productImage: DOMPurify.sanitize(item.productId.productImage || ""),
          },
        }));
        setCart(cartItems);
  ...
  const handleQuantityChange = async (index, change) => {
    const newQuantity = cart[index].quantity + change;
    if (newQuantity < 1) return toast.error("Quantity must be at least 1");
    if (newQuantity > cart[index].productId.productQuantity) return toast.error("Out of Stock");
  ...
  return (
    <div className="min-h-screen bg-[#fff8f2] p-6 text-[#472c1b]">
      <div className="max-w-5xl mx-auto">
        <h1 className="text-3xl font-bold mb-6 border-b pb-2 border-[#cc5f1c]">Your Book Cart</h1>
        {cart.length === 0 ? (
          ...
          <div className="space-y-6">
            {cart.map((item, index) => (
              ...
              >
                ...
                <div className="p-4 flex-1">
                  <h2 className="text-xl font-bold text-[#a04610]">
                    {item.productId.productName}
                  </h2>
                  <p className="text-[#935c44]">NPR. {item.productId.productPrice}</p>
                  <div className="flex items-center mt-3 space-x-3">
                    <button
                      onClick={() => handleQuantityChange(index, -1)}
                      className="bg-amber-300 px-2 py-1 rounded text-sm"
                    >
                      -
                    </button>
            ...
            <div className="bg-white rounded-lg shadow-md p-6 mt-6">
              <h2 className="text-2xl font-bold text-[#bd460d] mb-4">Subtotal: NPR. {subtotal}</h2>
              <Link to="/placeorder">
                <button className="w-full py-3 bg-gradient-to-r from-orange-500 to-red-500 text-white font-semibold rounded-lg shadow hover:from-orange-600 hover:to-red-600 flex items-center justify-center gap-2">
                  <span>Proceed to Checkout</span>
                  <ArrowRight size={20} />
                </button>
              </Link>
